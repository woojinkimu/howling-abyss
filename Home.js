  ...
  const handlePress = () => {
    axios.get(`https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/${nick}?api_key=${API_KEY}`)
      .then(res => {
        ...
        if (nick !== userData.name) {
          return (Alert.alert("닉네임을 정확하게 입력해주세요."));
        } else {
            ...
            .then(res => {
              ...
              for (let i = 0; i < matchList.length; i++) {
                ...
                axios.get(`https://kr.api.riotgames.com/lol/match/v4/matches/${gameId}?api_key=${API_KEY}`)
                    ...
                    for (let j = 0; j < identitiesData.length; j++) {
                      ...
                      if (nick === identity.player.summonerName) {
                        ...
                        teamObj['player'] = {
                          ...
                          gameDuration: Math.floor(matchData.gameDuration / 60),
                        };
                  ...
                  .catch(res => { if ('undefined' === typeof res.data || res.data.status >= 400) { Alert.alert('매치 데이터2 에러   ' + res); console.log(res); setIsButton(false);} });
              };
            })
            .catch(res => { if ('undefined' === typeof res.data || res.data.status >= 400) { Alert.alert('매치 데이터1 에러   ' + res); console.log(res); setIsButton(false);} });
        }
      })
      .catch(res => { if ('undefined' === typeof res.data || res.data.status >= 400) { Alert.alert('해당아이디 데이터가 없습니다.   ' + res); console.log(res); setIsButton(false);} });
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ ...styles.halfView }}>
        ...
        <TextInput style={styles.input} placeholder="닉네임" autoFocus={true} onChangeText={(text) => { setNick(text) }} />
